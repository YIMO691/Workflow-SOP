...
Development work leaves a durable record by default. Reuse the existing feature document; create a four-part feature note only when no suitable record exists. Small standalone fixes can use the original issue or PR. Update affected feature behavior/design in place and preserve valid manual content; do not generate separate task, requirements, design, test and delivery documents by default. Repository maintenance uses its existing PR and documentation as appropriate.
