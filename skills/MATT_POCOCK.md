...
原版 to-spec 会发布到配置的任务跟踪系统，并仍要求确认测试接口；to-tickets 也有任务发布与审批约定。采用本卡时，结果回现有功能说明或任务；不会因此自动新建 Issue、标签或外部发布。原版 domain-modeling 有 CONTEXT.md / ADR 的具体布局，本项目已有等价资料时引用即可。
