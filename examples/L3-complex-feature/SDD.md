...
| --- | --- | --- |
| QR-01 创建响应时间 | API 只校验并持久化任务，不同步生成文件 | AC-01 集成测试 |
| QR-02 数据安全 | 执行器按任务所有者身份查询；下载时重新鉴权 | AC-02/AC-03 安全测试 |
| QR-03 可恢复性 | 租约领取、固定对象键和有限重试 | 故障注入测试 |

## 权威 Owner 与读写边界

| 数据、状态或副作用 | 权威 Owner | 允许的读写方 | 失败边界 |
| --- | --- | --- | --- |
| 导出任务状态 | Task Store | API 创建/读取，Worker 条件更新 | 状态提交失败不得伪造成功 |
| 报表数据权限 | 现有权限服务 | API 和 Worker 只读校验 | 无法确认权限时拒绝导出 |
| 导出文件 | Object Storage | Worker 写入，下载服务签名读取 | 孤立文件由清理任务回收 |
