...
详细判断见 [完整流程](docs/WORKFLOW.md)。不需要先学习文档缩写或安装技能。**第一次使用**：准备目标项目、具体任务与本轮授权，按 [AI 接入说明](docs/AI_COLLABORATION.md#首次接入) 选择入口；项目已有约定就直接使用下面的任务提示。
...
Codex 与 Claude Code 的文件入口、普通聊天的使用方式和只读接入检查见 [首次接入](docs/AI_COLLABORATION.md#首次接入)。业务内容与运行证据留在目标项目允许的位置；本仓库的 AGENTS.md 只指导本仓库维护，不能直接复制成业务项目规范。
