...
```text
采用 Workflow-SOP：https://github.com/YIMO691/Workflow-SOP/blob/main/README.md 。
新任务在线读取 main 当前版本，本任务后续沿用同一版本；遵循项目已指定的版本策略。
从 docs/WORKFLOW.md 的“执行速查”开始，按 README 的读取表查涉及的细节。
先核对目标项目指令、代码和已有记录，在授权范围内完成任务；按速查说明实际结果与缺口。
任务：<描述要解决的问题，或提供已有提单>
```

**采用者默认在线读取，无需克隆仓库或每次手工拉取。** 将这段指引放进项目已有的 AI 入口后，新任务按指引读取更新；维护 SOP 本身才需要维护工作副本。更新与离线策略见 [线上读取与版本选择](docs/AI_COLLABORATION.md#线上读取与版本选择)。

当前仓库为私有，使用者及其 AI 读取工具需要获准访问；链接本身不授予权限。首次使用无须安装技能或部署服务，资料读不到时说明缺口，可使用获准提供的本地副本。
